...
export const getCoupon = async (_: Request, res: Response) => {
  try {
    ...
    if (!coupon) return res.status(404).json({ error: "No coupons available right now" });
...
export const applyCoupon = async (req: Request, res: Response) => {
  ...
  const { code } = req.body;
  try {
    if (code.trim() === "") return res.status(404).json({ error: "Enter Code" });
    ...
    if (!coupon) return res.status(404).json({ error: "sorry, an Error occurred" });

    if (coupon.code !== code) return res.status(404).json({ error: "sorry, Invalid code" });

    const now = new Date();

    if (coupon.expirationDate < now) return res.status(404).json({ error: "sorry, code is Expired" });
