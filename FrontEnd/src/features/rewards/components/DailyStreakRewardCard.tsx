...
import { claimDailyReward } from "../services";
...
const DailyStreakRewardCard = ({ dayInfo, nextRewardDay, handleUpdateNextTimerDay }: IProps) => {
  ...
  const collectDailyReward = async () => {
    ...
    try {
      setIsLoading(true);
      const data = await claimDailyReward(dayInfo.day);
      const updatedUser = {
        ...currentUser,
        points: data.points,
      ...
      dispatch(openToast({ message: `success collect ${dayInfo.reward} points`, type: "SUCESS" }));
    } catch (error) {
