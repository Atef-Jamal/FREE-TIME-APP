...
import { useInfinitePublicChatMsges } from "../hooks";
import { fetchSinglePublicMsg } from "../services";
...
const PublicChatBody = memo(() => {
  ...
  const [isLoadingSingleMsg, setIsLoadingSingleMsg] = useState(false);
  ...
  const fetchSingleMessage = useCallback(async () => {
    setIsLoadingSingleMsg(true);
    if (!messageIdParam) return;
    try {
      const message = await fetchSinglePublicMsg(messageIdParam);
      setOldMessage(message);
    } catch (error) {
      dispatch(openToast({ type: "ERROR_GENERAL", message: handleApiError(error) }));
    } finally {
      setIsLoadingSingleMsg(false);
    }
  }, [messageIdParam, dispatch]);
  ...
  }, [messageIdParam, setSearchParams]);

  useEffect(() => {
    if (!stopScrolling && !messageIdParam) {
      scrollToLastMessage();
    }
