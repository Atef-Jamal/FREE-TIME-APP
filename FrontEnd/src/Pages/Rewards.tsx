...
import Ladder from "../features/rewards/components/Ladder";
import DailyReward from "../features/rewards/components/DailyReward";
import { fetchBonusCode } from "../features/rewards/services";
import { IBounusCode } from "../features/rewards/types";
...
const Rewards = () => {
  ...
  const getBonusCode = async () => {
    if (userAuth !== "authenticated") {
      dispatch(openToast({ message: "Log In first", type: "ERROR_LOCK" }));
      return;
    }
    setLoading(true);
    try {
      const code = await fetchBonusCode();
      setBonusCode(code);
    } catch (error) {
  ...
  const copyBonusCode = () => {
    if (bonusCode) {
      ...
      dispatch(openToast({ message: "Copied!", type: "SUCESS" }));
    }
  };
