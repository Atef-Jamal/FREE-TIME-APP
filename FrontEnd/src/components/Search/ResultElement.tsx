...
const ResultElement = ({ type, results, searchTerm, emptyText }: TypeProps) => {
        ...
        <div key={item._id} className="py-1 px-2 flex items-center gap-3">
          ...
          {type === "FRAMES" && (
            <div className="relative w-10 h-10 sm:w-7 sm:h-7 object-fill ">
              ...
              <img alt={""} src={item.image} className="w-full h-full object-contain" />
            </div>
          )}
          {type === "APPS" && (
            <img
          ...
          {type === "MUSICS" && (
            <img alt={""} src={item.image} className="w-10 h-10 sm:w-7 sm:h-7 object-fill rounded-full" />
          )}
          {type === "USERS" && (
            <img alt={""} src={item.image} className="w-10 h-10 sm:w-7 sm:h-7 object-fill rounded-full" />
          )}
